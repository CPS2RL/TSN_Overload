...
def generate_packet_instances(model, flows, hyperperiod, fixed_classes):
    start_times      = {}
    is_scheduled     = {}
    packet_instances = []
    ...
    for flow in flows:
        ...
        for j in range(num_instances):
            ...
            queue_class = fixed_classes[pkt_id]

            s_var = model.addVar(vtype=GRB.INTEGER, lb=0, ub=hyperperiod, name=f"start_{pkt_id}")
            start_times[pkt_id] = s_var

            if queue_class == 8:
                is_scheduled[pkt_id] = model.addVar(vtype=GRB.BINARY, name=f"sched_{pkt_id}")
            else:
