...
def load_mapping(mapping_path):
    ...
    required = ['Packet_ID', 'Type']
    missing = [c for c in required if c not in map_df.columns]
    if missing:
        raise ValueError(f"Mapping file missing columns {missing}: {mapping_path}")

    return dict(zip(map_df['Packet_ID'].astype(str), map_df['Type'].astype(str)))


def process_single_file(file_path, output_folder, mapping_path):
    try:
        ...
        required_columns = ['Flow', 'Period', 'Deadline', 'Execution Time', 'Queue', 'm', 'k']
        ...
        pkt_type = load_mapping(mapping_path)
        ...
        for flow in flows:
            ...
            for j in range(num_instances):
                ...
                if pkt_id not in pkt_type:
                    print(f"ERROR: Packet '{pkt_id}' not found in mapping '{mapping_path}'")
                    return False
                queue_class = base_class if pkt_type[pkt_id] == 'M' else 8
