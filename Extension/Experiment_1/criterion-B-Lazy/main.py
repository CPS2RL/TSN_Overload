...
def get_mapping_path(file_path):
    results_folder = get_results_folder(file_path)
    base_filename = os.path.splitext(os.path.basename(file_path))[0]
    return os.path.join(results_folder, 'mapping', f'mapping_{base_filename}.csv')
...
def main():
    ...
    if os.path.isfile(path):
        ...
        mapping_path = get_mapping_path(path)
        os.makedirs(results_folder, exist_ok=True)
        print(f"Results will be saved to: '{results_folder}'")
        if process_single_file(path, results_folder, mapping_path):
            successful += 1
        else:
            failed += 1
        ...
        for i, csv_file in enumerate(csv_files, 1):
            ...
            results_folder = get_results_folder(file_path)
            mapping_path = get_mapping_path(file_path)
            os.makedirs(results_folder, exist_ok=True)
            print(f"\nProcessing {i}/{len(csv_files)}: {csv_file}")
            print(f"Results will be saved to: '{results_folder}'")
            if process_single_file(file_path, results_folder, mapping_path):
                successful += 1
            else:
                failed += 1
            gc.collect()
