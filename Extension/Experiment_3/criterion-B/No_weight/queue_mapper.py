        ...
        for i in range(len(q_pkts)):
            for j in range(i + 1, len(q_pkts)):
                ...
                if p["Arrival"] < q["Arrival"]:
                    ...
                    elif p_fp != q_fp:   p_first = p_fp < q_fp
                    else:                p_first = None

                    if p_first is True:
                        model.addConstr(start[pid] + p["Exec"] <= start[qid] + relax, name=f"tiebreak_pq_{pair_idx}")
                    elif p_first is False:
                        model.addConstr(start[qid] + q["Exec"] <= start[pid] + relax, name=f"tiebreak_qp_{pair_idx}")
                    else:
                        delta = model.addVar(vtype=GRB.BINARY, name=f"delta_{pair_idx}")
                        model.addConstr(start[pid] + p["Exec"] <= start[qid] + H * (1 - delta) + relax, name=f"order_pq_{pair_idx}")
                        model.addConstr(start[qid] + q["Exec"] <= start[pid] + H * delta + relax, name=f"order_qp_{pair_idx}")
                pair_idx += 1
