        ...
        if where == GRB.Callback.MIP:
            ...
            if self.iteration_count % self.check_interval == 0:
                ...
                if objbst != GRB.INFINITY and objbst != 0:
                    ...
                    print(f"Check #{len(self.gap_history)+1}: Gap: {current_gap*100:.3f}%, Solution: {int(objbst)}, Nodes: {int(nodcnt)}, Time: {runtime:.0f}s")
                    ...
                    if self.stable_count >= self.max_stable_iterations:
                        print(f"\nSTOPPING: Gap stable at {current_gap*100:.3f}% for {self.stable_count} consecutive checks")
...
def setup_objective(model, packet_instances, is_scheduled, df):
    ...
    if be_packet_count > 0:
        ...
        print("Flow weights:", {k_: v for k_, v in flow_weights.items()
                                 if any(pkt["Flow"] == k_ and pkt["Class"] == 8 for pkt in packet_instances)})
    else:
