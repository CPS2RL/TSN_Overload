...
def save_model_stats_to_csv(stats_dict, input_filename, results_folder, execution_time, optimizer="GCL", append=False):
    stats_df = pd.DataFrame([stats_dict])
    stats_df.insert(0, 'Optimizer', optimizer)
    stats_df['Input_File'] = input_filename
    stats_df['Solving_Time_Seconds'] = round(execution_time, 4)
    stats_df['Solving_Time_Minutes'] = round(execution_time / 60, 4)
    ...
    if append and os.path.exists(stats_filepath):
        existing_df = pd.read_csv(stats_filepath)
...
def handle_results(model, packet_instances, start_times, is_scheduled, execution_time, gap_history, stopping_reason, number_flows, input_filename, results_folder):
    ...
    if model.status in valid_solution_statuses and model.SolCount > 0:
        ...
        all_packets_df, gcl_df = process_results(model, packet_instances, start_times, is_scheduled, execution_time, stopping_reason, number_flows)
