    ...
    for i, pkt1 in enumerate(packet_instances):
        for j, pkt2 in enumerate(packet_instances[i+1:], start=i+1):
            ...
            if c1 != 8 and c2 != 8:
                ...
                model.addConstr(s1 + e1 + Cipg <= s2 + M * (1 - both_sched) + M * order, name=f"be_nonoverlap1_{stats['physical_constraints']}")
                model.addConstr(s2 + e2 + Cipg <= s1 + M * (1 - both_sched) + M * (1 - order), name=f"be_nonoverlap2_{stats['physical_constraints']}")

            elif c1 == 8 and c2 != 8:
                model.addConstr(s1 + e1 + max_execution_time <= s2 + M * (1 - sched1) + M * order, name=f"be_tt_nonoverlap1_{stats['physical_constraints']}")
                model.addConstr(s2 + e2 + Cipg <= s1 + M * (1 - sched1) + M * (1 - order), name=f"be_tt_nonoverlap2_{stats['physical_constraints']}")

            elif c1 != 8 and c2 == 8:
                model.addConstr(s1 + e1 + Cipg <= s2 + M * (1 - sched2) + M * order, name=f"tt_be_nonoverlap1_{stats['physical_constraints']}")
                model.addConstr(s2 + e2 + max_execution_time <= s1 + M * (1 - sched2) + M * (1 - order), name=f"tt_be_nonoverlap2_{stats['physical_constraints']}")
